...
RenamePageDialog::RenamePageDialog(wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style) : wxDialog(parent, id, title, pos, size, style)
{
	...
	pageNameSizer->Add(m_textCtrl3, 0, wxALL | wxEXPAND, 5);

	wxBoxSizer* nameDialogBtnSizer;
	nameDialogBtnSizer = new wxBoxSizer(wxHORIZONTAL);

	okDialogBtn = new wxBitmapButton(this, wxID_ANY, wxNullBitmap, wxDefaultPosition, wxDefaultSize, wxBU_AUTODRAW | wxBORDER_NONE);
	...
	cancelDialogBtn = new wxBitmapButton(this, wxID_ANY, wxNullBitmap, wxDefaultPosition, wxDefaultSize, wxBU_AUTODRAW | wxBORDER_NONE);
